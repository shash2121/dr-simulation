...
| **RDS** | MySQL 8.0 (db.t3.micro) | Primary datastore with cross-region replication |
...
This project uses **MySQL binlog replication** for cross-region data replication. No VPC peering is required — replication happens over AWS's internal backbone network.
...
- Both RDS parameter groups must have `binlog_format = ROW` and `binlog_backup_retention = 1`
...
### Step 2: Start Replication on DR
...
### Step 3: Point Primary as Replica of DR
...
- Update Secrets Manager in both regions with the new master endpoint (DR)
- Restart EKS pods to pick up new credentials:
  ```bash
...
### Step 3: Point DR as Replica of Primary
...
- Update Secrets Manager in both regions with the restored primary endpoint
...
| Feature | Behavior |
|---|---|
| **VPC Peering Required?** | No — uses AWS internal backbone |
| **Replication Type** | Binlog-based (physical) for MySQL |
| **DDL Replication** | ✅ Automatic (CREATE TABLE, ALTER TABLE, etc.) |
| **Replica State** | Read-only until promoted |
| **Failover Time** | ~5-15 minutes (promotion + availability check) |
| **Data Loss** | Minimal (async replication, usually seconds behind) |
...
- All instances use free-tier eligible types (`t3.micro`, `db.t3.micro`, `cache.t3.small`)
- RDS backup retention is `0` in DR region to minimize storage costs
